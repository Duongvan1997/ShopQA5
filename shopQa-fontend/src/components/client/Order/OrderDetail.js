...
const OrderDetail = () => {
  ...
  useEffect(() => {
    if (userData?.role === "CUSTOMER") {
      fetchOrderDetails(); // Fetch order details for the specific order ID
    } else {
      navigate("/");
    }
  }, [id]);
  ...
  return (
    <div className="TO_PAY">
      ...
      <Feedback visible={isModalVisible} hideModal={hideModal} orderData={orderData} fetchOrderDetails={fetchOrderDetails} />
    </div>
  );
};
